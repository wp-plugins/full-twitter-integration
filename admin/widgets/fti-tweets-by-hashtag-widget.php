...
class fti_tweets_by_hashtag_widget extends WP_Widget
{

    public function __construct()
    {
        parent::__construct(
            'fti_tweets_by_hashtag_widget',
            __('FTI Get tweets mentioning a Hashtag', 'fti_tweets_by_hashtag_widget_domain'),
            array('description' => __('Display tweets where your Hashtag was mentioned', 'fti_tweets_by_hashtag_widget_domain'),)
        );
    ...
    public function widget($args, $instance)
    {
        ...
        $views->display_widget_title($args, $instance, '#');

        $twitterClass = new Full_Twitter_Integration_Api();
        $tweetsArray = $twitterClass->get_tweets_by_hashtag( $instance['title'], $instance['max-num-tweets'] );

        $views->loop_tweets_for_widget($tweetsArray, $instance['title'], $instance);
    ...
    {
        $title = isset($instance['title'])? $instance['title'] : __('#Wordpress', 'fti_tweets_by_hashtag_widget_domain');
        $maxNumTweets = isset($instance['max-num-tweets'])? $instance['max-num-tweets'] : __(10, 'fti_tweets_by_hashtag_widget_domain');
        ...
        ?>
        <p>
            <label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Hashtag (Ex. #Wordpress):'); ?></label>
